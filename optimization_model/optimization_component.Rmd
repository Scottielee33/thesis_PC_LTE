...
optimize_upgrade <- function(perf_initial, lambda, perf_old_replaced, initial_period, average_performance, objective_function, get_perf_new, performance) {
  ...
  return(list(optimal_t_upgrade = optimal_t_upgrade, optimal_extended_period = optimal_extended_period, performance_data = performance_data))
}
```
...
```{r}
start_year <- 2014
start_quantile <- "First Quantile"
chosen_component <- "Disk"
...
cat("Starting performance for Chosen component:", cpu_perf$start_perf, "\n")
cat("Upgrade values for Chosen component:", paste(perf_upgrade_values, collapse = ", "), "\n")

# Parameters for optimization function
perf_initial <- total_starting_performance  # Initial performance of the chosen component
lambda <- 0.1                        # Decay rate (assumed)
perf_old_replaced <- cpu_perf$start_perf           # Initial performance of the old component (assumed)
