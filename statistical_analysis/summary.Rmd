...
gpu_summary <- GPU %>% group_by(Quartile) %>% summarize(AvgPrice = mean(AdjustedReleasePrice))

disk_summary <- Disk %>% group_by(Quartile) %>% summarize(AvgPrice = mean(AdjustedReleasePrice))

memory_summary <- Memory %>% group_by(Quartile) %>% summarize(AvgPrice = mean(AdjustedReleasePrice))
...
ggplot(combined_data, aes(x = Component, y = Price, fill = Quartile)) +
  ...
  labs(title = "Component Launch Prices per Quartile", x = "Component", y = "Price") +
